 ...
 * @version 0.1.0
 */
public class GeneratorIdTypeWorker extends GeneratorIdWorker {


    public GeneratorIdTypeWorker(long serverId) {
    ...
    public synchronized long nextId(long serviceId) {
        ...
        return ((timestamp - Constants.GOMEPLUS_EPOCH) << Constants.GOMEPLUS_TIMESTAMP_SHIFT) | (this.serverId <<
                ...
                Constants.GOMEPLUS_SERVICE_ID_SHIFT) | sequence;
    ...
    protected long getTimestamp(long timestamp, long lastTimestamp) {
        ...
        if (lastTimestamp == timestamp) {
            sequence = (sequence + 1) & Constants.GOMEPLUS_SEQUENCE_MASK;
            if (sequence == 0) {
